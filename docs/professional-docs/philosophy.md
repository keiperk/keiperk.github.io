...
## The goal-state method (the concrete "how" under "Always define success")
...
**Scope boundary:** this method applies to closed, transaction-based,
goal-oriented UX — something with a real "done" state to work backward
from. It does not cover trust/relationship elements (an honest status
message, a "here's what happens to your data" note — these don't collect
a property or gate a condition, they exist to make the relationship feel
safe) or open-ended, non-transactional UX (browsing, exploring, creating,
anything without a single goal state to decompose). Don't force the
method onto those — they're real UX work, just a different kind, governed
by "always build trust" instead.
