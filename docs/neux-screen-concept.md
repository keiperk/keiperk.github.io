...
| Path becomes primary view | Working the plan — single-column kanban lost the sense of sequence/dependency between milestones; Kevin wanted the linear path preserved, mirroring the record-launch-map artifact | Path strip upgraded from dots to real connected cards, made the default view; kanban demoted to a Path/Board toggle. Last card restyled as the goal (not a synthetic 10th node) | 2026-08-06 |
| *(next)* | | | |
